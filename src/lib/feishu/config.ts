// Feishu configuration
export const feishuConfig = {
  appId: "cli_a80d3c65433a5013", // 网页应用appId
  appSecret: "DlDy0kn4VCZP1n4l1Ugr1g7UYjFUlsO0", // 网页应用secret
  getUserAccessTokenPath: "/api/feishu/get_user_access_token", // 免登-获取user_access_token的api path
  getSignParametersPath: "/api/feishu/get_sign_parameters", // 鉴权-获取鉴权参数的api path
  noncestr: "SeMa6DS8NX7iAHPmknSfKRn6t3rY6Xfb", // 随机字符串，用于鉴权签名用
  apiPort: "8989", // 后端指定端口
};
